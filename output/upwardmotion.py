...
# Motion settings (premium feel)
BODY_DRIFT_UP_PX = 120   # total upward drift across the video duration
...
# =============================
# RENDER FRAME (WITH MOTION)
# =============================
def build_frame_renderer(hook, body):
    # Preload fonts once
    ...
    base_bg = create_gradient_rgba()

    # Precompute hook layout
    tmp = Image.new("RGBA", (WIDTH, HEIGHT))
    tmp_draw = ImageDraw.Draw(tmp)

    hook_lines = wrap_text(tmp_draw, hook, hook_font, WIDTH * 0.9)
    hook_height = len(hook_lines) * (HOOK_FONT_SIZE + 10) + 30

    # Precompute body wrap
    body_lines = wrap_text(tmp_draw, body, body_font, WIDTH * 0.85)

    # Precompute watermark metrics
    wm_height = 90
    wm_y = HEIGHT - wm_height - 60

    def render(t):
        # progress 0..1
        p = min(max(t / SLIDE_DURATION, 0.0), 1.0)

        # Body drift: starts slightly lower and moves up smoothly
        ...
        # ---------- BODY (moves up) ----------
        # Base body position (same as before) then subtract drift
        ...
        draw.text((wm_x, wm_y + 18), WATERMARK_TEXT, font=watermark_font, fill=(255, 255, 255))
...
make_frame = build_frame_renderer(hook, body)

clip = VideoClip(make_frame, duration=SLIDE_DURATION).set_fps(FPS)
...
print("✅ Motion short created:", OUTPUT_VIDEO)
